...
function getCredentialKeys(rawCredentials) {
  return rawCredentials.split(" ").map((entry) => entry.split(":")[0]);
}
...
function readCredentialKeys() {
  ...
  do {
    ...
    if (readLine === "" || readLine === undefined)
      return getCredentialKeys(line.trim());
    line += `${readLine} `;
  } while (true);
}
...
function doWork() {
  ...
  do {
    ...
    if (credentialKeys === null) return valid;
    let credentialsValid = true;
    requirements.forEach((requirement) => {
      if (!credentialKeys.includes(requirement)) credentialsValid = false;
    });
