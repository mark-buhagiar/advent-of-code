...
function parseInput() {
  ...
  return inputs.map((item) => item.split(/\n/g).map(x=>new Set(x)));
}
...
function doWork(input) {
  ...
  return input.reduce((total, item) => {
    let currSet = item[0];
    if (item.length === 1) return currSet.size + total;
    for (let i = 1; i < item.length; i++) {
      currSet = new Set([...currSet].filter(x => item[i].has(x)));
    }
    return total + currSet.size;
  }, 0);
}

performance.mark("processing-start");
console.log(doWork(parseInput()));
