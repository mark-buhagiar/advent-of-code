...
function doWork(input, target) {
  let runningTot = 0;
  input.forEach((element) => {
    const parts = element.match(/(.+) contain (.+)./, "g");
    const wrapper = parts[1].slice(0, -1);
    if (!wrapper.includes(target)) return;
    if (parts[2] === "no other bags") {
      return;
    }
    const suitcases = parts[2].split(",");
    suitcases.forEach((suitcase) => {
      const suitcaseParts = suitcase.match(/(\d)+ (.+)/, "g");
      runningTot +=
        suitcaseParts[1] * doWork(input, suitcaseParts[2].slice(0, -1));
    });
  });
  return runningTot + 1;
}
...
console.log(doWork(input.split(/\n/), "shiny gold bag") - 1); // we subtract 1 becasue the outermost case doesn't count
