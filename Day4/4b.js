...
function getCredential(rawCredentials) {
  return rawCredentials.split(" ").reduce((obj, entry) => {
...
function readCredential() {
  if (curLine >= input.length) return null;

  let line = "";
  do {
    const readLine = input[curLine];
    curLine++;
    if (readLine === "" || readLine === undefined)
      return getCredential(line.trim());
    line += `${readLine} `;
  } while (true);
}

function isValidField(credential, requirement) {
  if (!credential.hasOwnProperty(requirement)) return false;
  try {
    const value = credential[requirement];
    switch (requirement) {
      case "byr":
      ...
      case "hgt": {
        const actualValue = value.substring(0, value.length - 2);
        if (value.endsWith("in")) {
          return parseInt(actualValue) >= 59 && parseInt(actualValue) <= 76;
        } else if (value.endsWith("cm")) {
          return parseInt(actualValue) >= 150 && parseInt(actualValue) <= 193;
        }
        return false;
      }
      case "hcl": {
...
function doWork() {
  ...
  do {
    ...
    if (credential === null) return valid;
    let credentialsValid = true;
    requirements.forEach((requirement) => {
      if (!isValidField(credential, requirement)) {
