...
function isValid(policyCountMin, policyCountMax, policyRequirement, password) {
  var requirementCountInPassword = (
    password.match(new RegExp(policyRequirement, "g")) || []
  ).length;
  return (
    requirementCountInPassword >= policyCountMin &&
    requirementCountInPassword <= policyCountMax
  );
}
...
function doWork() {
  ...
  input.forEach((element) => {
    const inputParts = element.split(" ");

    const policyCountParts = inputParts[0].split("-");
    const policyCountMin = policyCountParts[0];
    const policyCountMax = policyCountParts[1];

    const policyRequirement = inputParts[1].substring(
      0,
      inputParts[1].length - 1
    );

    if (
      isValid(policyCountMin, policyCountMax, policyRequirement, inputParts[2])
    )
      validCount++;
  });
